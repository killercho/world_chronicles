...
RAYGUIAPI void GuiSetAlpha(
    float
        alpha);  // Set gui controls alpha (global state), alpha goes from 0.0f to 1.0f
...
RAYGUIAPI void
GuiSetStyle(int control, int property, int value);     // Set one style property
...
RAYGUIAPI char**        GuiLoadIcons(
           const char* fileName,
           bool
               loadIconsName);  // Load raygui icons file (.rgi) into internal icons data
RAYGUIAPI void
...
RAYGUIAPI int
              GuiTabBar(Rectangle    bounds,
                        const char** text,
                        int          count,
                        int* active);  // Tab Bar control, returns TAB to be closed or -1
RAYGUIAPI int GuiScrollPanel(Rectangle   bounds,
                             const char* text,
                             Rectangle   content,
                             Vector2*    scroll,
...
RAYGUIAPI int GuiDropdownBox(Rectangle   bounds,
                             const char* text,
                             int*        active,
                             bool        editMode);  // Dropdown Box control
RAYGUIAPI int GuiSpinner(Rectangle   bounds,
                         const char* text,
                         int*        value,
                         int         minValue,
                         int         maxValue,
...
RAYGUIAPI int
GuiValueBoxFloat(Rectangle   bounds,
                 const char* text,
                 char*       textValue,
                 float*      value,
                 bool        editMode);  // Value box control for float values
RAYGUIAPI int
GuiTextBox(Rectangle bounds,
           char*     text,
           int       textSize,
           bool      editMode);  // Text Box control, updates input text

RAYGUIAPI int GuiSlider(Rectangle   bounds,
                        const char* textLeft,
                        const char* textRight,
                        float*      value,
                        float       minValue,
                        float       maxValue);  // Slider control
RAYGUIAPI int GuiSliderBar(Rectangle   bounds,
                           const char* textLeft,
                           const char* textRight,
                           float*      value,
                           float       minValue,
                           float       maxValue);  // Slider Bar control
RAYGUIAPI int GuiProgressBar(Rectangle   bounds,
                             const char* textLeft,
                             const char* textRight,
                             float*      value,
                             float       minValue,
...
RAYGUIAPI int GuiGrid(Rectangle   bounds,
                      const char* text,
                      float       spacing,
                      int         subdivs,
...
RAYGUIAPI int
              GuiMessageBox(Rectangle   bounds,
                            const char* title,
                            const char* message,
...
RAYGUIAPI int GuiTextInputBox(
    ...
    int         textMaxSize,
    bool*
        secretViewActive);  // Text Input Box control, ask for text, supports secret
RAYGUIAPI int
              GuiColorPicker(Rectangle   bounds,
                             const char* text,
                             Color* color);  // Color Picker control (multiple color controls)
RAYGUIAPI int GuiColorPanel(Rectangle   bounds,
                            const char* text,
                            Color*      color);  // Color Panel control
RAYGUIAPI int GuiColorBarAlpha(Rectangle   bounds,
                               const char* text,
                               float*      alpha);  // Color Bar Alpha control
RAYGUIAPI int GuiColorBarHue(Rectangle   bounds,
                             const char* text,
...
RAYGUIAPI int GuiColorPickerHSV(
    ...
    const char* text,
    Vector3*
        colorHsv);  // Color Picker control that avoids conversion to RGB on each call (multiple color controls)
RAYGUIAPI int GuiColorPanelHSV(
    Rectangle   bounds,
    const char* text,
    Vector3*
        colorHsv);  // Color Panel control that updates Hue-Saturation-Value color value, used by GuiColorPickerHSV()
//----------------------------------------------------------------------------------------------------------
...
static void DrawRectangle(
    int x, int y, int width, int height, Color color);  // -- GuiDrawRectangle()
...
static Texture2D LoadTextureFromImage(
    Image
        image);  // -- GuiLoadStyle(), required to load texture from embedded font atlas image
static void SetShapesTexture(
    Texture2D tex,
    Rectangle
        rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
static const char* GetDirectoryPath(
    const char*
        filePath);  // -- GuiLoadStyle(), required to find charset/font file from text .rgs

static int* LoadCodepoints(
    const char* text,
    int*
        count);  // -- GuiLoadStyle(), required to load required font codepoints list
...
static const char* CodepointToUTF8(
    int codepoint,
    int*
        byteSize);  // Encode codepoint into UTF-8 text (char array size returned as parameter)
...
static void
GuiDrawRectangle(Rectangle rec,
                 int       borderWidth,
                 Color     borderColor,
                 Color color);  // Gui draw rectangle using default raygui style

static const char**
GuiTextSplit(const char* text,
             char        delimiter,
             int*        count,
...
// NOTE: This define is also used by GuiMessageBox() and GuiTextInputBox()
    ...
    Rectangle statusBar = {
        bounds.x, bounds.y, bounds.width, (float)statusBarHeight
    };
...
int GuiGroupBox(Rectangle bounds, const char* text) {
    ...
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){
            bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height },
        0,
        BLANK,
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x,
                     ...
                     GetColor(GuiGetStyle(DEFAULT,
                                          (state == STATE_DISABLED) ?
                                              (int)BORDER_COLOR_DISABLED :
                                              (int)LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + bounds.width - 1,
                                                 bounds.y,
                                                 RAYGUI_GROUPBOX_LINE_THICK,
                                                 bounds.height },
                     0,
...
int GuiLine(Rectangle bounds, const char* text) {
    ...
    Color color = GetColor(GuiGetStyle(DEFAULT,
                                       (state == STATE_DISABLED) ?
                                           (int)BORDER_COLOR_DISABLED :
                                           (int)LINE_COLOR));
    ...
    if (text == NULL)
        ...
        GuiDrawRectangle(
            RAYGUI_CLITERAL(Rectangle){ bounds.x,
...
#define RAYGUI_TABBAR_ITEM_WIDTH 160
    ...
    Rectangle tabBounds = {
        bounds.x, bounds.y, RAYGUI_TABBAR_ITEM_WIDTH, bounds.height
    };
    ...
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){
            bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 },
        0,
        BLANK,
...
int GuiScrollPanel(Rectangle   bounds,
                   const char* text,
                   Rectangle   content,
                   Vector2*    scroll,
                   Rectangle*  view) {
    ...
    if ((state != STATE_DISABLED) && !guiLocked) {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds)) {
            ...
            if (hasHorizontalScrollBar &&
                ...
                scrollPos.y +=
                    wheelMove * mouseWheelSpeed.y;  // Vertical scroll
        }
    }
    ...
    if (state == STATE_NORMAL) {
        GuiDrawRectangle(
            ...
            GetColor(GuiGetStyle(
                TOGGLE,
                ((*active) ? BORDER_COLOR_PRESSED : (BORDER + state * 3)))),
            GetColor(GuiGetStyle(
                TOGGLE,
                ((*active) ? BASE_COLOR_PRESSED : (BASE + state * 3)))));
        GuiDrawText(text,
                    GetTextBounds(TOGGLE, bounds),
                    GuiGetStyle(TOGGLE, TEXT_ALIGNMENT),
                    GetColor(GuiGetStyle(TOGGLE,
                                         ((*active) ? TEXT_COLOR_PRESSED :
                                                      (TEXT + state * 3)))));
    } else {
        GuiDrawRectangle(bounds,
                         GuiGetStyle(TOGGLE, BORDER_WIDTH),
...
int GuiCheckBox(Rectangle bounds, const char* text, bool* checked) {
    ...
    if (*checked) {
        ...
        GuiDrawRectangle(
            check, 0, BLANK, GetColor(GuiGetStyle(CHECKBOX, TEXT + state * 3)));
    }
...
int GuiDropdownBox(Rectangle   bounds,
                   const char* text,
                   int*        active,
                   bool        editMode) {
    int      result = 0;
    ...
    GuiDrawText(items[itemSelected],
                GetTextBounds(DROPDOWNBOX, bounds),
    ...
    if (editMode) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (i == itemSelected) {
                ...
                GuiDrawText(
                    items[i],
                    GetTextBounds(DROPDOWNBOX, itemBounds),
                    GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
...
int GuiTextBox(Rectangle bounds, char* text, int textSize, bool editMode) {
    ...
    if ((state != STATE_DISABLED) &&  // Control not disabled
        !GuiGetStyle(TEXTBOX,
                     TEXT_READONLY) &&  // TextBox not on read-only mode
        ...
        if (editMode) {
            ...
            if ((!multiline && IsKeyPressed(KEY_ENTER)) ||
                (!CheckCollisionPointRec(mousePosition, bounds) &&
                 IsMouseButtonPressed(MOUSE_LEFT_BUTTON))) {
                textBoxCursorIndex =
                    0;  // GLOBAL: Reset the shared cursor index
                autoCursorCounter =
    ...
    GuiDrawText(text + textIndexOffset,
                textBounds,
...
int GuiSpinner(Rectangle   bounds,
               const char* text,
               int*        value,
               int         minValue,
               int         maxValue,
    ...
    //--------------------------------------------------------------------
    result = GuiValueBox(
        valueBoxBounds, NULL, &tempValue, minValue, maxValue, editMode);
...
int GuiValueBox(Rectangle   bounds,
                const char* text,
                int*        value,
                int         minValue,
                int         maxValue,
...
// NOTE: Requires static variables: frameCounter
int GuiValueBoxFloat(Rectangle   bounds,
                     const char* text,
                     char*       textValue,
                     float*      value,
                     bool        editMode) {
...
int GuiSlider(Rectangle   bounds,
              const char* textLeft,
              const char* textRight,
              float*      value,
              float       minValue,
    ...
    GuiDrawRectangle(
        ...
        GetColor(GuiGetStyle(SLIDER,
                             (state != STATE_DISABLED) ? BASE_COLOR_NORMAL :
                                                         BASE_COLOR_DISABLED)));
...
int GuiSliderBar(Rectangle   bounds,
                 const char* textLeft,
                 const char* textRight,
                 float*      value,
                 float       minValue,
...
int GuiProgressBar(Rectangle   bounds,
                   const char* textLeft,
                   const char* textRight,
                   float*      value,
                   float       minValue,
...
int GuiDummyRec(Rectangle bounds, const char* text) {
    ...
    GuiDrawText(
        ...
        TEXT_ALIGN_CENTER,
        GetColor(GuiGetStyle(BUTTON,
                             (state != STATE_DISABLED) ? TEXT_COLOR_NORMAL :
                                                         TEXT_COLOR_DISABLED)));
    //------------------------------------------------------------------
...
// List View control
int GuiListView(Rectangle   bounds,
                const char* text,
                int*        scrollIndex,
                int*        active) {
    int          result    = 0;
...
int GuiColorPanel(Rectangle bounds, const char* text, Color* color) {
    ...
    Vector3 vcolor = { (float)color->r / 255.0f,
                ...
                GuiDrawRectangle(
                    ...
                    ((x + y) % 2) ?
                        Fade(GetColor(GuiGetStyle(COLORPICKER,
                             ...
                             0.4f) :
                        Fade(GetColor(
                                 GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)),
                             0.4f));
            }
        }
...
// Message Box control
int GuiMessageBox(Rectangle   bounds,
                  const char* title,
                  const char* message,
                  const char* buttons) {
    ...
    int result =
        -1;  // Returns clicked button from buttons list, 0 refers to closed window button
...
int GuiTextInputBox(Rectangle   bounds,
    ...
    if (secretViewActive != NULL) {
        static char stars[] = "****************";
        if (GuiTextBox(
                RAYGUI_CLITERAL(Rectangle){ textBoxBounds.x,
                                            textBoxBounds.y,
...
// Grid lines alpha amount
    ...
    if (subdivs > 0) {
        ...
        for (int i = 0; i < linesV; i++) {
            Rectangle lineV = {
                bounds.x + spacing * i / subdivs, bounds.y, 1, bounds.height + 1
            };
            GuiDrawRectangle(
                lineV,
                0,
        ...
        for (int i = 0; i < linesH; i++) {
            Rectangle lineH = {
                bounds.x, bounds.y + spacing * i / subdivs, bounds.width + 1, 1
            };
            GuiDrawRectangle(
                lineH,
                0,
...
void GuiLoadStyleDefault(void) {
    ...
    GuiSetStyle(
        DEFAULT, BACKGROUND_COLOR, 0xf5f5f5ff);  // DEFAULT specific property
    GuiSetStyle(
        DEFAULT, TEXT_LINE_SPACING, 15);  // DEFAULT, 15 pixels between lines
    GuiSetStyle(
        DEFAULT,
    ...
    if (guiFont.texture.id != GetFontDefault().texture.id) {
        ...
        SetShapesTexture(guiFont.texture,
                         RAYGUI_CLITERAL(Rectangle){ whiteChar.x + 1,
                                                     whiteChar.y + 1,
                                                     whiteChar.width - 2,
                                                     whiteChar.height - 2 });
    }
...
const char* GuiIconText(int iconId, const char* text) {
    ...
    return NULL;
#else
    static char buffer[1'024]  = { 0 };
    static char iconBuffer[16] = { 0 };

    if (text != NULL) {
        memset(buffer, 0, 1'024);
        snprintf(buffer, 1'024, "#%03i#", iconId);

        for (int i = 5; i < 1'024; i++) {
            buffer[i] = text[i - 5];
            if (text[i - 5] == '\0')
                break;
        }
    ...
    if (rgiFile != NULL) {
        ...
        if ((signature[0] == 'r') && (signature[1] == 'G') &&
            ...
            if (loadIconsName) {
                ...
                fseek(
                    rgiFile, iconCount * RAYGUI_ICON_MAX_NAME_LENGTH, SEEK_CUR);
...
char** GuiLoadIconsFromMemory(const unsigned char* fileData,
                              int                  dataSize,
                              bool                 loadIconsName) {
        ...
        if (loadIconsName) {
            ...
            for (int i = 0; i < iconCount; i++) {
                guiIconsName[i] =
                    (char*)RAYGUI_MALLOC(RAYGUI_ICON_MAX_NAME_LENGTH);
                memcpy(
                    guiIconsName[i], fileDataPtr, RAYGUI_ICON_MAX_NAME_LENGTH);
    ...
    if ((signature[0] == 'r') && (signature[1] == 'G') &&
        ...
        if (fontDataSize > 0) {
            ...
            if ((fontImageCompSize > 0) &&
                ...
                imFont.data = DecompressData(
                    compData, fontImageCompSize, &dataUncompSize);
            ...
            if (font.texture.id != 0) {
                ...
                if ((recsDataCompressedSize > 0) &&
                    ...
                    font.recs =
                        (Rectangle*)DecompressData(recsDataCompressed,
                                                   recsDataCompressedSize,
                                                   &recsDataUncompSize);
                ...
                if ((glyphsDataCompressedSize > 0) &&
                    ...
                    unsigned char* glyphsDataUncomp =
                        DecompressData(glypsDataCompressed,
                                       glyphsDataCompressedSize,
                    ...
                    for (int i = 0; i < font.glyphCount; i++) {
                        memcpy(&font.glyphs[i].value,
                               glyphsDataUncompPtr,
                               sizeof(int));
                        memcpy(&font.glyphs[i].offsetX,
                               glyphsDataUncompPtr + 4,
                               sizeof(int));
                        memcpy(&font.glyphs[i].offsetY,
                               glyphsDataUncompPtr + 8,
                               sizeof(int));
                        memcpy(&font.glyphs[i].advanceX,
                               glyphsDataUncompPtr + 12,
                               sizeof(int));
                ...
                font =
                    GetFontDefault();  // Fallback in case of errors loading font atlas texture
...
GuiTextSplit(const char* text, char delimiter, int* count, int* textRow) {
    ...
    #define RAYGUI_TEXTSPLIT_MAX_TEXT_SIZE 1'024
#endif
...
static Color GuiFade(Color color, float alpha) {
    ...
    Color result = {
        color.r, color.g, color.b, (unsigned char)(color.a * alpha)
    };
